...
function getHotel(hotelID) {
  return ajax
    .post(HOTEL_DETAIL, {hotelid: hotelID})
    .then(response => {
...
function saveHotel(params) {
  const payload = {
    ...
    isgroup: false,
    refcity: [],
    priority: params.isPriority === true
      ? 1
      : 0,
  ...
  return ajax
    .post(HOTEL_ADD_EDIT, payload)
    .then(response => {
      if (response.code === SUCCESS) {
        return {code: SUCCESS, hotelID: response.data}
      } else if (response.code === ERROR) {
        return UNKNOW_ERROR
      } else if (response.code === TIMEOUT) {
        return NETWORK_TIMEOUT
